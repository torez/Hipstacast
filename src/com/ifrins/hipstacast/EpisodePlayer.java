...
public class EpisodePlayer extends Activity {
    ...
    private void setupUIFromLocalData() {
    	...
    	if (show_id > 0) {
            Cursor p = getContentResolver().query(Uri.parse("content://com.ifrins.hipstacast.provider.HipstacastContentProvider/podcasts/"+show_id+"/episodes"), 
					new String[] {"_id", "title", "duration", "podcast_id", "status", "position", "content_url", "shownotes"}, "_id = ?", new String[] {String.valueOf(podcast_id)}, null);
            p.moveToFirst();
            ...
            ((TextView)findViewById(R.id.playerEpisodeName)).setText(p.getString(p.getColumnIndex("title")));
            seek.setMax(p.getInt(p.getColumnIndex("duration"))*1000);
            
            Log.d("HIP-TOT", String.valueOf(p.getInt(p.getColumnIndex("duration"))));
            WebView v = (WebView)findViewById(R.id.playerEpisodeDesc);
            v.loadData(p.getString(p.getColumnIndex("shownotes")), "text/html", null);
            isPlaying = false;
            mHandler.removeCallbacks(mUpdateTimeTask);
            mHandler.postDelayed(mUpdateTimeTask, 1000);
            buildNotification(p.getString(p.getColumnIndex("title")));
            p.close();
            if (!fromNotification) complete = true;
    	}
    }
